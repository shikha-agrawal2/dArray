...
public class LongestPalindromicSubsequence {
    ...
    public int calculateRecursive(char str[],int start,int len){
        //each char is palindrome of length 1
        if(len == 1){
            return 1;
        }
        if(len ==0){
        ...
        if(str[start] == str[start+len-1]){  //len-1= last index
            return 2 + calculateRecursive(str,start+1,len-2);  //as last 2 characters are same
        }else{
            return Math.max(calculateRecursive(str, start+1, len-1), calculateRecursive(str, start, len-1));  // decrement length by 1 always
        }
    }
